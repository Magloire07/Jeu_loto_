...
public class LeJeuLoto extends javax.swing.JFrame {
    ...
    public void initLots(){
        // création de 4 lot
          Lot  l1=new Objet("AirPod","Objet",2,Toolkit.getDefaultToolkit().getImage(getClass().getResource("/Images/airpod.jpeg")));// creationd'un lot de type objet
          Lot  l2=new Objet("Ordinateur","Objet",3,Toolkit.getDefaultToolkit().getImage(getClass().getResource("/Images/ordinateur.jpg")));// creationd'un lot de type objet
    ...
    private void afficheCartes(){
        ...
        for(Joueur j: LesJ.getLst()){
            ...
            switch(nbCartes){ // après chaque case le compteur est incrémenté de 2 pour passer aux joueur suivant(ses deux Jpanel reservé pour ses cartes)
                case 0: cpt+=2; break;
                case 1: 
     ...
     */
        ...
        for(Joueur j:LesJ.getLst()){ // boucle sur la liste des joueurs
            if(j.getPseudo().equals(ps)) 
             jou=j; // affectation du joueur à la variable jou si son pseudo est reconnue dans la liste des joueur de la classe LeJeuLoto
        }
      ...
      AchatCarteDlg ac= new AchatCarteDlg(this,true,LesJ,nbLig,nbCol,nbNum);
        ac.setVisible(true);
         ...
         if(indic){ // si la carte est acheté on l'ajoute et l'affiche devant le joueur
           this.carteAchetee=true;// on met à true  chaque fois qu'on s'en vas acheter pourque si on vien le faire parcequ'on n'as pas pu démarré le jeu la variable redient à true avant de  refaire le test dans la gestionnaire de démarrage
          afficheCartes();
          this.setSize(this.getWidth()+1,this.getHeight()+1);// pour forcé le rafraichisement de la fenêtre 
           ...
           JOptionPane jop1 = new JOptionPane();
            jop1.showMessageDialog(null, "veillez choisir deux joueur au moins avant", "Information", JOptionPane.INFORMATION_MESSAGE);
    ...
    private void btTirageActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btTirageActionPerformed
        ...
        if(Gagnants.getTaille()>0){// il y a aumoins un gagnant 
            if(Gagnants.getTaille()>1){ //il y a plus d'un gagnant
                 gagnant_retenu= (int)(Math.random()*(Gagnants.getTaille()+1));// donc on tire au hasard le gagnant final
                 gagnant=Gagnants.getJoueur(gagnant_retenu); // mis a joueur du gagnant de la classe 
                 switch(option){
                     case 1:this.historiqueTir.append("\n... "+gagnant.getPseudo()+" QUINE "+gagnant.getPseudo()+" a gagné "+this.Nom_cadeau+" \n la Partie Terminée ");this.btTirage.setEnabled(false);  break;
                     case 2:this.historiqueTir.append("\n... "+gagnant.getPseudo()+" DOUBLE QUINE "+gagnant.getPseudo()+" a gagné "+this.Nom_cadeau+"\n la Partie Terminée ");this.btTirage.setEnabled(false);break;
                     case 3:this.historiqueTir.append("\n... "+gagnant.getPseudo()+" CARTON PLEIN " +gagnant.getPseudo()+" a gagné "+this.Nom_cadeau+" \n la Partie Terminée");this.btTirage.setEnabled(false);
                 ...
                 switch(option){
                     case 1:this.historiqueTir.append("\n... "+Gagnants.getJoueur(0).getPseudo()+" QUINE "+Gagnants.getJoueur(0).getPseudo()+" a gagné "+this.Nom_cadeau+" \n la Partie Terminée ") ;this.btTirage.setEnabled(false);  break;
                     case 2:this.historiqueTir.append("\n... "+Gagnants.getJoueur(0).getPseudo()+" DOUBLE QUINE "+Gagnants.getJoueur(0).getPseudo()+" a gagné "+this.Nom_cadeau+"\n la Partie Terminée ");this.btTirage.setEnabled(false);break;
                     case 3:this.historiqueTir.append("\n... "+Gagnants.getJoueur(0).getPseudo()+" CARTON PLEIN " +Gagnants.getJoueur(0).getPseudo()+" a gagné "+this.Nom_cadeau+" \n la Partie Terminée");this.btTirage.setEnabled(false);
    ...
    public boolean choisirLot(){
        ...
        if(this.carteAchetee)// s'il y a des cartes acheter  et au moins un joueur
        {
            ...
            JOptionPane jop1 = new JOptionPane();
            jop1.showMessageDialog(null, "veillez  que chaque joueur achete aumoins une carte avant de demarrer", "Information", JOptionPane.INFORMATION_MESSAGE);
            return false;
         }
         }
        else{
            JOptionPane jop1 = new JOptionPane();
            jop1.showMessageDialog(null, "IL n'y a plus de lot disponible ", "Information", JOptionPane.INFORMATION_MESSAGE);
           return false;
        }
        return true;
    }
    ...
    private void continuerActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_continuerActionPerformed
       ...
       this.demarrer.setEnabled(false); // on désactiv le bouton démarrer
          ...
          JOptionPane jop1 = new JOptionPane();
          jop1.showMessageDialog(null, "Vous ne pouvez pas continuer , Carton plein atteint", "Information", JOptionPane.INFORMATION_MESSAGE); 
       } 
    ...
    private void recommencerActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_recommencerActionPerformed
        ...
        for(Joueur j: this.LesJ.getLst()){
           j.setSolde(j.getSolde()+j.getMesCartes().getTaille()*j.getMesCartes().getCarte(0).getPrix()); // on reinitialise le solde du joueur en fonction du nombre de carte qu'il a 
