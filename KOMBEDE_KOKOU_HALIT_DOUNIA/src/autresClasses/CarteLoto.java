...
public class CarteLoto {
 ...
 public double getPrix() { return this.prix; } 
    ...
    public void setNbNumeros(int nbNumeros) {
        if(nbNumeros>=3&& nbNumeros<=tab.length){ // affect un nombre de numéro  supérieur à 3 det inférieur à la taille du de la carte de LOTO 
...
// déclaration de la méthode d'initialisation des valeur de la carte de loto 
 ...
 { 
     ...
     // affection de (nbNumeros) aléatoir de numéro au tableau sachant qu'il n'y est pas encore 
         for(int k=0; k<this.nbNumeros; k++){
             ...
             tab[i][j]=val;
             i=(i+1)%this.nbLig;  // incrementation de la variable d'indice de ligne  qui ne dépasse jamais le nombre le ligne du tableau (revient à 0 )
               ...
               for(CarteLoto c: j.getMesCartes().getLst()){ // parcours de la liste des carte du joueur courant
                   for(int i=0; i<3; i++){// on boucle sur trois ligne 
                     for(int k=0; k<c.getNbCol(); k++){// on boucle sur la colonne de chaque ligne
                         // le tableau n'est pas vide et sa valeur n'est pas egale à celle du jeton au même indice
                        if(c.getTab()[i][k]!=0&&(c.getTab()[i][k]!=c.getJetons()[i][k])) // si la premiere  ou la deuxieme  ou la troisieme ligne du jeton ne correspond celle de la carte
                         egaux=false; 
                     }
                     if(egaux)
                         nbreLine++;
                     
                     egaux=true;// on réinitialise car lors qu'un boucle egaux peux valoir false  mais c'est pas forcement le cas pour pour toute les line ei qu'il peut existé une autre ligne qui ne modifie pas la valeur de égaux et cette ligne nous intéresse
                  }
